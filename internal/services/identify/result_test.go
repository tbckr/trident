...
func TestResult_IsEmpty(t *testing.T) {
	assert.True(t, (&identify.Result{}).IsEmpty())
	assert.False(t, (&identify.Result{
		Detections: []identify.Detection{{Type: "CDN", Provider: "AWS CloudFront", Evidence: "foo.cloudfront.net."}},
	}).IsEmpty())
}
