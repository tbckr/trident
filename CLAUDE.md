...
**`"aggregate"` cobra group** — commands that orchestrate multiple services (not wrapping a single external API) use `GroupID: "aggregate"`. Register with `cmd.AddGroup(&cobra.Group{ID: "aggregate", Title: "Aggregate Commands:"})` after `"osint"` and before `"utility"` in `newRootCmd`. Always registered (not conditional like `"aliases"`). See `internal/cli/apex.go`.
