...
**`httpclient.New()` signature:** `New(proxy, userAgent string, logger *slog.Logger, debug bool) (*req.Client, error)` — pass `nil, false` in tests/non-debug paths. When `debug && logger != nil`, attaches `OnAfterResponse` hook that logs method, URL, and status code at DEBUG level; on non-2xx responses also logs a body snippet (up to 512 bytes). No `EnableTraceAll()` — no timing fields. Proxy precedence: explicit `proxy` arg → `client.SetProxyURL(proxy)`; empty `proxy` → `client.SetProxy(http.ProxyFromEnvironment)` (honours `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` env vars at request time). `req.Client.SetProxy` accepts `func(*http.Request) (*url.URL, error)` — same signature as `http.ProxyFromEnvironment`.
...
**`sync.WaitGroup.Go`** — available since Go 1.25 (project uses Go 1.26); use `wg.Go(func() { ... })` instead of `wg.Add(1)` + `go func() { defer wg.Done() }()`. Loop variable capture is automatic in Go 1.22+ so no `i := i` shadowing is needed.
...
**`output.NewWrappingTable`** — shared factory in `internal/output/terminal.go`; use for plain (ungrouped) tables. **`output.NewGroupedWrappingTable`** — use when rows are grouped by a type column (e.g. DNS): merges repeated first-column cells (`MergeHierarchical`) and draws separator lines between groups (`BetweenRows: tw.On`); requires `"github.com/olekukonko/tablewriter/renderer"` imported in `terminal.go`. Overhead values: 20 for 2-column tables, 6 for 1-column tables. **Merging is consecutive-only** — the same value appearing non-contiguously creates separate merged groups. Sort rows before calling `Bulk` when parallel-gathered records for the same host may be interleaved. **`output.NewGroupedWrappingTablePerCol`** — use instead of `NewGroupedWrappingTable` when the host/key column contains variable-length content (e.g. apex subdomain names like `_sipfederationtls._tcp.example.com`); `Global` cap applies to ALL columns equally so a wide host column causes terminal overflow even when overhead is set correctly for the value column. Accepts `map[int]int{colIdx: maxWidth}` (omit a column or set 0 = unconstrained). Proportional width pattern for 3-col apex table: `const overhead = 18; avail := output.TerminalWidth(w) - overhead; hostMax := max(20, min(30, avail*2/5)); valueMax := max(20, avail-hostMax)`.

**Display-only sort in `WriteTable`** — when a service runs parallel queries that may interleave records for the same host, sort a copy inside `WriteTable` (not in `Run`/`AggregateResults`) to keep JSON/text output order stable. Pattern: `sorted := make([]Record, len(records)); copy(sorted, records); sort.Slice(sorted, ...)`. Use a sort-key function with string prefix `"0:"` for the primary input, `"2:"` for sentinel rows (e.g. `detected`), `"1:"` for everything else.
...
**`services.MultiResultBase[T, PT]`** — generic base in `internal/services/multi.go`; `PT multiItem[T]` constrains the element type (`*T` + `IsEmpty() bool` + `WriteText(io.Writer) error`). Provides `IsEmpty`, `MarshalJSON`, `WriteText`; embed it and add `WriteTable` to complete a service's `MultiResult`. **`MarshalJSON` output format** — serializes as a bare JSON array (`[{...}, {...}]`), not a `{"results": [...]}` envelope; test with `var arr []map[string]any`.

**`runServiceCmd`** — shared `RunE` body in `internal/cli/root.go`; enforces the service PAP gate then delegates to `runCmdBody` (input resolution + single/bulk paths). Each regular subcommand's `RunE` calls `runServiceCmd(cmd, d, svc, args)`. **`runAggregateCmd`** — use instead of `runServiceCmd` for `AggregateService` implementations; enforces `MinPAP` gate (not `PAP`). Both produce the same error format: `%q requires PAP %s but limit is %s`. **`runCmdBody`** — the shared post-gate execution body called by both.
...
**Aggregate `Result.Skipped` field** — when a sub-service is not run due to PAP, populate `result.Skipped` with the sub-service name. `WriteText` renders each entry as `[skipped: <name>]\n`; `WriteTable` appends rows with `Host="skipped"` after `sortRecordsForDisplay` (not integrated into the sort key function). JSON tag: `"skipped,omitempty"`.

**`writeServicesTable` adaptive columns** — in `internal/cli/services.go`; renders a single "PAP" column (using `MinPAP`) when all entries have `MinPAP == PAP`; splits to "Min PAP" / "Max PAP" columns only when any entry differs. Overhead: 30 for 3-col (equal), 37 for 4-col (split). JSON always includes both `min_pap` and `max_pap` fields for machine consumers.
...
- **golangci-lint v2 config structure:** `linters-settings` → `linters.settings`; `formatters-settings` → `formatters.settings`; `issues.exclude-rules` → `linters.exclusions.rules`. `goimports.local-prefixes` is an array (not a string). `gosimple` is merged into `staticcheck` — do not list it separately. `staticcheck` is bundled in golangci-lint — no separate CI job needed.
...
- `latest-deps.yml` — weekly Monday 06:00 UTC: upgrades direct deps only (`go get <direct>@latest` + `go mod tidy` + `go test ./...`) in gVisor sandbox; indirect deps are intentionally resolved from direct dep constraints, not force-upgraded independently. **Do not use `go get -u`** — the `-u` flag also upgrades all transitive dependencies, which breaks the direct-only intent. Local equivalent: `scripts/upgrade-deps.sh` (same logic, does NOT run tests — run `go test ./...` after).
