...
- `release.yml` — GoReleaser + SBOM + Cosign (tag push); permissions scoped to job level (`contents: write`, `id-token: write`, `attestations: write`)
  - GitHub-native provenance: `actions/attest-build-provenance` generates GitHub Artifact Attestation from `checksums.txt` (recognized by OpenSSF Scorecard)
  - Custom SLSA provenance: generated by `scripts/generate-provenance.sh` (env vars only; no `artifacts.json` dependency); subject is `checksums.txt`; no `byproducts` field — all artifacts (archives + SBOMs) are covered by `checksums.txt` directly; signed with `cosign attest-blob` (keyless OIDC); cert identity: `https://github.com/tbckr/trident/.github/workflows/release.yml@refs/tags/<VERSION>`; issuer: `https://token.actions.githubusercontent.com`
